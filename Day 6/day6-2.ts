...
function main() {
    ...
    const time: number = parseInt([...[...data.matchAll(/Time:[0-9\s]+/g)][0][0].matchAll(/[0-9]+/g)].map((m) => m[0]).reduce((a: string, b: string) => a + b));
    const distance: number = parseInt([...[...data.matchAll(/Distance:[0-9\s]+/g)][0][0].matchAll(/[0-9]+/g)].map((m) => m[0]).reduce((a: string, b: string) => a + b));

    console.log('total:', getValue(time, distance))
}
